...
- يضيف اختبار القناة عادة `/v1/chat/completions` مرة واحدة، ويستخدم العنوان كما هو في وضع الرابط الكامل، مع حفظ زمن الاستجابة.
...
- إحصاءات لمدة 3 ساعات ويوم و7 و15 يوما والإجمالي، مع قوائم حسب النموذج واسم API Key والمستخدم.
...
يشغل `wrangler.toml` العامل كل ساعة بالتعبير `0 * * * *`. يفحص القنوات كل 60 دقيقة وينتظر رابط الاختبار الوحيد حتى 60 ثانية. يجمع الاستخدام ويرسله عند تفعيل التنبيهات كل 360 دقيقة (6 ساعات).
...
يضاف `/v1/chat/completions` مرة واحدة فقط ولا تجرب نهايات أخرى. عند تحديد الرابط الكامل يستخدم الاختبار وطلبات الإكمال العنوان المدخل كما هو. يحفظ الرابط وزمن الاستجابة، وتظل مزامنة النماذج تستخدم الرابط الأساسي مع `/v1/models`.
