...
- 模型广场支持编辑显示名、复制和隐藏模型。
- 邮箱验证注册改为 13 位数字验证码，13 分钟有效、3 次输入机会、67 秒重发冷却。
- 系统设置支持邮箱后缀验证和 QQ 邮箱强制数字前缀，默认都开启。
...
启用邮箱验证时，注册不再发送链接邮件，而是发送 13 位数字验证码邮件。

- 验证码 13 分钟内有效。
- 每个验证码最多可输入 3 次。
- 验证码输入页有“没收到？重新发送”按钮。
- 重发验证码有 67 秒冷却。
- 注册页面包含确认密码输入框，避免密码一次打错。
- 邮箱后缀验证默认开启，支持 `qq.com`、`163.com`、`gmail.com`、`outlook.com`、`yeah.net`、`hotmail.com`、`126.com` 等常见邮箱。
- QQ 邮箱默认必须使用纯数字 QQ 号前缀。
- 管理员可在系统设置中关闭邮箱后缀验证或 QQ 邮箱数字前缀要求。
