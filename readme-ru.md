...
- Регистрация с проверкой email использует 13-значный цифровой код: он действует 13 минут, допускает 3 попытки ввода и повторную отправку после 67 секунд.
- Проверка домена email и требование цифрового префикса для QQ-почты переключаются в системных настройках и включены по умолчанию.
...
Если проверка email включена, регистрация больше не отправляет ссылку. Вместо этого на почту отправляется 13-значный цифровой код.

- Код действует 13 минут.
- У каждого кода есть 3 попытки ввода.
- На странице ввода кода есть кнопка повторной отправки.
- Повторная отправка имеет задержку 67 секунд.
- В форме регистрации есть поле подтверждения пароля.
- Проверка домена email включена по умолчанию и поддерживает распространенные домены, например `qq.com`, `163.com`, `gmail.com`, `outlook.com`, `yeah.net`, `hotmail.com` и `126.com`.
- Для адресов `qq.com` по умолчанию требуется числовой префикс QQ.
- Администратор может отключить проверку домена и требование числового QQ-префикса в системных настройках.
