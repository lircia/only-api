...
- Проверка канала обычно один раз добавляет `/v1/chat/completions`; в режиме полного URL адрес используется без изменений, а задержка сохраняется.
...
- Статистика за 3 часа, 1 день, 7 дней, 15 дней и все время, а также списки по моделям, именам API Key и пользователям.
...
`wrangler.toml` запускает Worker каждый час выражением `0 * * * *`. Каналы проверяются каждые 60 минут, единственный тестовый URL ожидается до 60 секунд. Использование собирается и при включенных уведомлениях отправляется каждые 360 минут (6 часов).
...
Обычно `/v1/chat/completions` добавляется один раз без перебора других окончаний. В режиме полного URL тест и реальные Completion-запросы используют введенный адрес без изменений. URL и задержка сохраняются; синхронизация моделей использует Base URL плюс `/v1/models`.
