...
Бывало ли у вас, что вы хотите использовать API какого-либо провайдера или платформы, но его endpoint имеет большую задержку локально или иногда недоступен? Этот проект может частично решить такую проблему, перенаправляя API-запросы через Cloudflare.
