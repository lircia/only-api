...
| `CF_API_TOKEN` | Переменная | требуется Account Analytics Read |
...
Используйте Account ID, а не Zone ID. Для `CF_API_TOKEN` требуется `Account > Account Analytics > Read`. Неудачный запрос не сохраняется и не отправляется как нулевое использование.
