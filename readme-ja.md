...
- OpenAI 互換 `/v1/*` 転送。
- ユーザーごとの利用上限は設定しません。
- チャネルテストは通常 `/v1/chat/completions` を一度だけ補完し、完全 URL モードでは入力をそのまま使い、成功 URL と遅延を記録します。
...
- 3 時間、1 日、7 日、15 日、全期間の統計と、モデル、API Key 名、ユーザー別一覧。
...
`wrangler.toml` の `0 * * * *` が Worker を毎時起動します。チャネル確認は既定 60 分で、単一のテスト URL は最大 60 秒待ちます。Workers 利用量は 360 分（6 時間）ごとに取得し、通知有効時は同じ間隔で送信します。
...
通常は Base URL に `/v1/chat/completions` を一度だけ補完し、他の末尾は試しません。「完全 URL」を選ぶとテストと実際の補完呼び出しで入力 URL をそのまま使います。成功 URL と遅延は保存され、モデル同期は引き続き Base URL と `/v1/models` を使います。
